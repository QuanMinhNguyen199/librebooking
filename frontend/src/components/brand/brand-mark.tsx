export function BrandMark({ compact = false, inverse = false }: { compact?: boolean; inverse?: boolean }) {
  return <div className="flex items-center gap-3">
    ...
    {!compact && <div><p className={`brand-font text-lg font-bold tracking-wide ${inverse ? "text-white" : "text-[#171717]"}`}>THEHE<span className="text-[#df1f2d]">GEO</span></p><p className={`text-[11px] ${inverse ? "text-white/45" : "text-slate-400"}`}>Quản lý tài nguyên</p></div>}
  </div>;
}
