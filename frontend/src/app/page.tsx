...
const nav = [
  { label: "Tổng quan", icon: LayoutGrid }, { label: "Lịch sử dụng", icon: CalendarDays },
  { label: "Tài nguyên", icon: Boxes },
      ...
      <section className="lg:pl-64">
        ...
        <div className="mx-auto max-w-[1500px] p-4 md:p-8">
          ...
          <div className="mt-5 grid gap-5 xl:grid-cols-[1.4fr_1fr]">
            ...
            <article id="booking-calendar" className={`scroll-mt-24 rounded-2xl border bg-white p-5 transition md:p-6 ${active === "Lịch sử dụng" ? "border-[#df1f2d] shadow-[0_0_0_3px_rgba(223,31,45,.08)]" : "border-slate-200"}`}><div className="mb-5 flex items-center justify-between"><div><h2 className="text-lg font-bold uppercase tracking-wide">Lịch sử dụng của tôi</h2><p className="mt-1 text-xs text-slate-400">{reservations.length} lượt đăng ký sắp tới</p></div><CalendarDays size={19} className="text-[#c71927]" /></div><div className="max-h-64 space-y-5 overflow-auto pr-1">{reservations.map(item => <div key={item.id} className="flex gap-3"><div className="w-11 text-xs font-semibold text-slate-500">{item.time}</div><div className={`mt-1 h-9 w-1 rounded-full ${item.colorClass}`} /><div><p className="text-sm font-semibold">{item.title}</p><p className="mt-1 text-xs text-slate-400">{item.resourceName}</p></div></div>)}</div><button onClick={() => setBookingOpen(true)} className="mt-6 w-full rounded-xl border border-dashed border-slate-300 py-2.5 text-xs font-semibold text-slate-500 hover:border-[#df1f2d] hover:text-[#c71927]">+ Đăng ký thêm</button></article>
          </div>
        </div>
      </section>
