...
import { ArrowUpRight, Bell, Bot, Boxes, Building2, CalendarDays, CalendarPlus, Car, ChartNoAxesCombined, ChevronDown, CircleDot, Clock3, Gauge, Laptop, LayoutGrid, Menu, MoreHorizontal, Plus, Search, Settings, UserRoundCog, X } from "lucide-react";
...
const nav = [
  { label: "Tổng quan", icon: LayoutGrid }, { label: "Đặt tài nguyên", icon: CalendarPlus },
  { label: "Lịch của tôi", icon: CalendarDays }, { label: "Tài nguyên", icon: Boxes },
  { label: "AI Usage", icon: Gauge },
...
export default function Home() {
  ...
  function navigate(label: string) {
    setActive(label);
    setMobileOpen(false);
    if (label === "Đặt tài nguyên") {
      setBookingOpen(true);
    }
    if (label === "Lịch của tôi") {
      ...
      <section className="lg:pl-64">
        ...
        <div className="mx-auto max-w-[1500px] p-4 md:p-8">
          ...
          <div className="grid gap-4 sm:grid-cols-2 xl:grid-cols-4">{[
            ["Tài nguyên hoạt động", "128", "+6 tháng này", Building2, "text-[#c71927] bg-red-50"], ["Đang được sử dụng", "42", "67% công suất", Clock3, "text-blue-700 bg-blue-50"], ["Chi phí tháng này", "184,6 tr", "−8,4% so tháng trước", ChartNoAxesCombined, "text-violet-700 bg-violet-50"], ["Claude hôm nay", "$38.42", "12,8M tokens", Bot, "text-amber-700 bg-amber-50"],
          ...
          <div className="mt-5 grid gap-5 xl:grid-cols-[1.4fr_1fr]">
            ...
            <article id="booking-calendar" className={`scroll-mt-24 rounded-2xl border bg-white p-5 transition md:p-6 ${active === "Lịch của tôi" ? "border-[#df1f2d] shadow-[0_0_0_3px_rgba(223,31,45,.08)]" : "border-slate-200"}`}><div className="mb-5 flex items-center justify-between"><div><h2 className="text-lg font-bold uppercase tracking-wide">Lịch của tôi</h2><p className="mt-1 text-xs text-slate-400">{reservations.length} booking sắp tới</p></div><CalendarDays size={19} className="text-[#c71927]" /></div><div className="max-h-64 space-y-5 overflow-auto pr-1">{reservations.map(item => <div key={item.id} className="flex gap-3"><div className="w-11 text-xs font-semibold text-slate-500">{item.time}</div><div className={`mt-1 h-9 w-1 rounded-full ${item.colorClass}`} /><div><p className="text-sm font-semibold">{item.title}</p><p className="mt-1 text-xs text-slate-400">{item.resourceName}</p></div></div>)}</div><button onClick={() => { setActive("Đặt tài nguyên"); setBookingOpen(true); }} className="mt-6 w-full rounded-xl border border-dashed border-slate-300 py-2.5 text-xs font-semibold text-slate-500 hover:border-[#df1f2d] hover:text-[#c71927]">+ Đặt thêm tài nguyên</button></article>
          </div>
        </div>
      </section>
