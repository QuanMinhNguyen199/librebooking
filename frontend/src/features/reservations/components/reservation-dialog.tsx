...
export function ReservationDialog({ open, onClose, onCreated, onError }: { open: boolean; onClose: () => void; onCreated: (reservation: ReservationSummary) => void; onError: (action: string, error: unknown) => void }) {
  ...
  return <div className="fixed inset-0 z-[60] grid place-items-center bg-black/45 p-4 backdrop-blur-sm" onMouseDown={event => { if (event.target === event.currentTarget) onClose(); }}>
    <section className="w-full max-w-xl overflow-hidden rounded-2xl bg-white shadow-2xl">
      <header className="flex items-start justify-between border-b border-slate-100 px-6 py-5"><div><p className="text-xs font-semibold uppercase tracking-widest text-[#c71927]">Booking mới</p><h2 className="mt-1 text-2xl font-bold uppercase tracking-wide">Đặt tài nguyên</h2><p className="mt-1 text-sm text-slate-500">Chọn thời gian và tài nguyên phù hợp cho công việc.</p></div><button onClick={onClose} className="rounded-lg p-2 text-slate-400 hover:bg-slate-100" aria-label="Đóng"><X size={19} /></button></header>
      {completed ? <div className="grid min-h-80 place-items-center p-8 text-center"><div><CheckCircle2 size={52} className="mx-auto text-[#df1f2d]" /><h3 className="mt-4 text-xl font-bold">Đã tạo booking</h3><p className="mt-2 text-sm text-slate-500">Lịch hôm nay đã được cập nhật.</p></div></div> : <form onSubmit={submit} className="space-y-5 p-6">
